#!/usr/bin/env python3
"""
Extract words by brute-force validation of all letter combinations.
For lengths 2-5 letters, this is feasible.

This approach doesn't enumerate the DAWG - it validates every possible
letter combination against the DAWG structure.
"""
...
def main():
    print("="*60)
    print("Brute Force Word Extraction (No Reference Lists)")
    print("="*60)
    ...
    all_words = set()
    output_file = "/Volumes/T7/retrogames/oldmac/maven_re/lexica/maven_brute_extracted.txt"

    # Process lengths 2-5 (feasible)
    max_length = 5
    if len(sys.argv) > 1:
    ...
    print(f"\nTotal: {len(all_words)} words (2-{max_length} letters)")

    # Verify known words
    test = ['aa', 'ab', 'cat', 'act', 'the', 'be', 'at', 'ta']
    print("\nVerification:")
    for w in test:
        if len(w) <= max_length:
