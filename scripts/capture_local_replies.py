#!/usr/bin/env python3
"""Capture CODE37+548 local reply generation around a naturally proposed move."""
...
try:
 ...
 def state():
  out={name:read(a5-off,n).hex() for name,off,n in [('board',0x4302,544),('values',0x40e2,1088),('counts',0x5ab2,128),('undo',0xaca,33),('sorted_rack',0x3204,8),('leave_values',0x440c,256),('row_flags',0x6d2,32)]}
  ...
  return out
 fixed={name:read(a5-off,n).hex() for name,off,n in [('letter_values',0x6bee,256),('word_multipliers',0x684e,544),('letter_multipliers',0x6a6e,544),('letter_class',0x428,128),('bit_masks',0x662a,128)]};fixed['alphabet']=read(long(a5-0x662e),28).hex();fixed['bingo_bonus']=word(a5-0x65aa)
 report=dict(scope=__doc__,a5=a5,code_sha256=hashlib.sha256(code).hexdigest(),fixed=fixed,calls=[])
 ...
 for i in range(a.count):
  ...
  while True:
   ...
   if pc==caller:break
   assert pc==callback;call['candidates'].append(dict(move=read(long(g[15]+4),34).hex(),new_tiles=word(a5-0x4e2a)))
   remove(pc);r.command('s');arm(pc)
  clear();call.update(final=state(),final_own=read(own,8).hex(),final_other=read(other,8).hex());report['calls'].append(call);print('Captured',len(call['candidates']),'replies',flush=True)
