#!/usr/bin/env python3
"""Controlled debugger test of the original random sample-limit exit.

Waits for a natural completed batch, then lowers ONLY the disposable config's
32-bit sample limit to its already-computed first-candidate weight. This is
explicit configuration perturbation, not an unmodified UI termination trace.
No code, counters, return values, board, racks or exception state are written.
"""
...
try:
 ...
 arm(base+0x65a);print('Waiting for natural completed simulation batch',flush=True);r.command('c');assert regs()[17]==base+0x65a;remove(base+0x65a)
 frame=regs()[14]
 for _ in range(20):
 ...
 sites={base+0x672:'limit_reached',base+0xd44:'exception_caught',base+0xd86:'before_restore',base+0xdd6:'board_racks_restored',base+0xde2:'leave_rebuilt',caller:'returned'}
 for addr in sites:arm(addr)
 original_config=read(config,24+46*count);assert word(config+16)==0
 ...
 after_config=read(config,len(original_config));assert after_config==changed+original_config[4:]
 report['controlled_mutation']=dict(address=config,field='signed32 sample limit',before=original_config[:4].hex(),after=changed.hex(),first_candidate_weight=weight,all_other_config_bytes_unchanged=True)
 report['initial_after_mutation']=state();a.output.write_text(json.dumps(report,indent=2)+'\n')
