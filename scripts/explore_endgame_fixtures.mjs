...
const name='endgame-seven-search-live';
...
const wasm=fs.readFileSync('.build/maven-endgame.wasm');const module=await WebAssembly.compile(wasm);assert.deepEqual(WebAssembly.Module.imports(module),[]);async function probe(ownRack,otherRack){const e=(await WebAssembly.instantiate(module,{})).exports;
...
[j.initial.row_zero_count,j.initial.new_tiles,...j.initial.rows,...j.initial.columns,j.initial.ranking.cutoff,j.initial.reserve_control,j.budget,j.start_ticks,j.fixed.bingo_bonus].forEach((v,i)=>e.maven_endgame_set(i,v));

put('own',Buffer.from(ownRack+'\0'));put('other',Buffer.from(otherRack+'\0'));put('elapsed_ticks',Array(4096).fill(0));e.maven_endgame_set(9,0);
let trapped=false;try{e.maven_endgame_run();}catch(error){if(!(error instanceof WebAssembly.RuntimeError))throw error;trapped=true;}
return {own:ownRack,other:otherRack,iterations:e.maven_endgame_get(8),free_head:e.maven_endgame_get(11),poll_calls:e.maven_endgame_get(14),empty_pool_polls:e.maven_endgame_get(15),trapped};}
const pool='bcinrsueeekoqt'.split('').sort(),racks=new Set();
function combos(start,word){if(word.length===7){racks.add(word);return;}for(let i=start;i<pool.length;++i)combos(i+1,word+pool[i]);}combos(0,'');
const selected=[...racks].sort((a,b)=>hash(Buffer.from(a)).localeCompare(hash(Buffer.from(b)))).slice(0,600),results=[];
for(const own of selected){let other=pool.join('');for(const c of own)other=other.replace(c,'');const result=await probe(own,other);results.push(result);if(result.trapped||result.empty_pool_polls>0)console.log(JSON.stringify(result));}
results.sort((a,b)=>b.iterations-a.iterations);fs.writeFileSync('analysis/toolchain/endgame-fixture-exploration.json',JSON.stringify({scope:'Exploratory reconstructed-wasm fixture selection only; poll_calls are general polling, not collection counts; empty_pool_polls indicate pool exhaustion observations; no original-runtime correctness claim. A trap can indicate the bounded 4096-clock bridge limit or an engine diagnostic.',results},null,2)+'\n');console.log(JSON.stringify(results.slice(0,8),null,2));
