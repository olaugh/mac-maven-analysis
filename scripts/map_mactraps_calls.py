#!/usr/bin/env python3
"""Inventory candidate A5-relative calls to the fingerprinted MacTraps library.
...
def main():
    ...
    mapping=json.loads((root/'analysis/toolchain/mactraps-symbols.json').read_text())
    targets={e['a5_callable_offset']:e for e in mapping['maven_code34_entries']}
    rows=[]
    for path in sorted((root/'resources/CODE').glob('*.bin')):
        ...
        for offset in range(4,len(data)-3,2):
            ...
            rows.append(dict(code_resource=rid,resource_offset=offset,
                             ...
                             a5_slot=slot,names=targets[slot]['names'],target_resource_offset=targets[slot]['maven_offset'],
                             context_start=max(4,offset-12),
                             context_hex=data[max(4,offset-12):offset+16].hex(),
                             code_sha256=hashlib.sha256(data).hexdigest()))
    counts=[dict(a5_slot=slot,names=targets[slot]['names'],target_resource_offset=e['maven_offset'],
                 candidates=sum(r['a5_slot']==slot for r in rows))
            for slot,e in targets.items()]
    print(json.dumps(dict(scope='Aligned JSR/JMP opcode candidates only; no instruction-boundary or reachability proof. Names come from full library interval comparisons; aliases remain unresolved.',
