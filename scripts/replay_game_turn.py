#!/usr/bin/env python3
"""Replay a complete human commit with totals, history payload, evaluated application, statistics, refill and side/end transition."""
import argparse
import hashlib
import json
...
def main():
    ...
    turn=j['turn'];collector_final=j['final'];final=turn['final']
    ...
    declarations=[]
    assert len(turn['refills'])==1
    refill=turn['refills'][0]
    ...
    rawarray('rack1',bytes.fromhex(turn['initial']['rack1']))
    rawarray('display_order',(ROOT/'resources/prfs/0_0.bin').read_bytes()[0x312:0x32e])
    rawarray('expected_bag',bytes.fromhex(refill['final']['bag_workspace']) or b'\0')
    rawarray('expected_rack1',bytes.fromhex(final['rack1']))
    array('totals',turn['initial']['totals'],'uint32_t');array('expected_totals',final['totals'],'uint32_t')
    declarations.append('static const uint32_t random_events[][2]={'+(','.join('{'+str({'ticks':0,'private_random':1,'toolbox_random':2}[e['kind']])+','+str(e['value'])+'u}' for e in refill['events']) or '{0,0}')+'};')
    declarations.append('static uint32_t private_seed='+str(refill['private_seed'])+'u;')
...
'''+ '\n'.join(declarations)+'''
static size_t random_index,history_calls;
static uint32_t next(unsigned kind){assert(random_index<RANDOM_COUNT);assert(random_events[random_index][0]==kind);return random_events[random_index++][1];}
static uint32_t ticks(void *u){(void)u;return next(0);}
static uint32_t private_random(void *u){uint32_t x;(void)u;x=maven_private_random_next(&private_seed);assert(x==next(1));return x;}
static int16_t toolbox_random(void *u){uint32_t x;(void)u;x=next(2);return x<32768?(int16_t)x:(int16_t)((int32_t)x-65536);}
static int append(void *u,int tag,const uint8_t *payload,size_t length){(void)u;
 assert(tag==2&&length==52&&!memcmp(payload,move,34)&&payload[34]==0&&payload[35]==1);
 assert(!strcmp((const char *)payload+36,(const char *)rack));assert(!strcmp((const char *)payload+44,(const char *)rack1));++history_calls;return 1;
}
...
int main(void){MavenApplyState app={0};MavenRackBalanceCache balance={0};MavenMoveEvaluation state={0};MavenMoveEvaluationResult result;MavenAppliedMove workspace={{0},0};uint32_t features[22];MavenDisplayMoveScore display_score;MavenGameTurn game={0};int16_t ids[4096],weights[4096];
...
'''.replace('PATTERN_COUNT',str(j['pattern_record_count'])).replace('RANDOM_COUNT',str(len(refill['events'])))
    ...
    source+='game.evaluation=&state;game.racks[0]=rack;game.racks[1]=rack1;game.display_order=display_order;game.display_score=&display_score;memcpy(game.totals,totals,sizeof totals);memcpy(game.statistics[0],statistics,sizeof statistics);game.append=append;game.refill=(MavenRefillOps){0,private_random,toolbox_random,ticks};\n'
    source+=f'game.initial_stack_ticks={refill["stack_ticks"]}u;\n'
    source+='CHECK(maven_commit_game_turn(&game,move,ids,weights,&workspace),1);CHECK(diagnostics,0);CHECK(history_calls,1);result=game.evaluation_result;memcpy(features,game.features,sizeof features);\n'
    source+=f'CHECK(random_index,{len(refill["events"])});CHECK(private_seed,{refill["final"]["private_seed"]}u);CHECK(game.bag_length,{len(bytes.fromhex(refill["bag"]))});CHECK(game.selected_side,{final["selected_side"]});CHECK(game.phase,{final["phase"]});\n'
    source+='if(!check("totals",game.totals,expected_totals,sizeof totals)||!check("rack1",rack1,expected_rack1,8)||!check("refill bag",game.bag,expected_bag,game.bag_length))return 1;\n'
    source+='if(!check("game statistics",game.statistics[0],expected_statistics,sizeof statistics))return 1;\n'
    ...
    source+='return 0;}\n'
    build=ROOT/'.build';build.mkdir(exist_ok=True);probe=build/'game-turn-probe.c';probe.write_text(source);exe=build/'game-turn-probe'
    sources=['game_turn','rack_masks','rack_refill','evaluated_application','evaluation_features','display_move_score','move_statistics','move_evaluation','apply_move','board_state','move_finalize','score_move','score_accumulate','place_letters','rack_counts','remaining_tiles','undo_move','premium_exposure','rack_balance','rack_composition','pattern_match','pattern_lookup','letter_expectation']
    ...
    print(json.dumps(dict(scope='Human commit with totals, history payload semantic fields, computed collector/display scoring, application/statistics, real refill and side/end state; prepared engine tables supplied; file save and subsequent final rack adjustment excluded',contributions=len(expected),result_bits=j['result_bits'],all_matched=True)))
