#!/usr/bin/env python3
"""Replay original CODE37 local-reply masks, apply/enumerate/undo and raw callbacks."""
import argparse,hashlib,json,struct,subprocess
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]
p=argparse.ArgumentParser(description=__doc__);p.add_argument('--capture',type=Path,default=ROOT/'analysis/toolchain/local-replies-03-live.json');a=p.parse_args();j=json.loads(a.capture.read_text());assert j['complete'];data_path=ROOT/'../../media/maven/session/share/maven2.1';assert hashlib.sha256(data_path.read_bytes()).hexdigest()==j['dictionary_sha256'];assert hashlib.sha256((ROOT/'resources/CODE/37_37.bin').read_bytes()).hexdigest()==j['code_sha256']
decl=[];checks=[]
...
for i,c in enumerate(j['calls']):
 ...
 source+=f'maven_generate_local_replies(&local,{prefix}move,{prefix}own,{other_argument});CHECK(diagnostics,0);CHECK(failed,0);CHECK(candidate_index,candidate_count);\n'
 ...
 source+=f'if(!check("sorted rack",local.sorted_rack,{prefix}final_sorted_rack,8))return 1;\n'
...
build=ROOT/'.build';build.mkdir(exist_ok=True);probe=build/'local-replies-probe.c';probe.write_text(source);exe=build/'local-replies-probe'
