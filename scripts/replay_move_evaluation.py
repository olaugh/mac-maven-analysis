#!/usr/bin/env python3
"""Replay a complete original collector through the assembled portable C modules."""
import argparse
import hashlib
import json
...
def main():
    parser=argparse.ArgumentParser(description=__doc__);parser.add_argument('--capture',type=Path,default=ROOT/'analysis/toolchain/move-evaluation-live.json');a=parser.parse_args()
    ...
    source='''#include "move_evaluation.h"
...
int main(void){MavenApplyState app={0};MavenRackBalanceCache balance={0};MavenMoveEvaluation state={0};MavenMoveEvaluationResult result;int16_t ids[4096],weights[4096];
...
'''.replace('PATTERN_COUNT',str(j['pattern_record_count']))
    ...
    source+='result=maven_evaluate_move(move,&state,ids,weights);CHECK(diagnostics,0);\n'
    ...
    source+='return 0;}\n'
    build=ROOT/'.build';build.mkdir(exist_ok=True);probe=build/'move-evaluation-probe.c';probe.write_text(source);exe=build/'move-evaluation-probe'
    ...
    print(json.dumps(dict(scope='Complete collector replay: total, every emitted ID/weight, board/value/rack/count/undo/scorer and balance-cache state; prepared cache and decoded score tables supplied',contributions=len(expected),result_bits=j['result_bits'],all_matched=True)))
