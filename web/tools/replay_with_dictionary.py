...
Copy of scripts/replay_original_magpie_matches.py with a --dictionary option so a
recompiled DAWG can be checked against the recorded original rankings."""
import argparse,ctypes as C,gzip,json,struct,sys
from pathlib import Path
sys.path.insert(0,str(Path(__file__).resolve().parents[2]/'scripts'))
from play_magpie_matches import ROOT,Position,Ranking,CALL,build

def main():
 p=argparse.ArgumentParser(description=__doc__);p.add_argument('capture');p.add_argument('--build',action='store_true');p.add_argument('--dictionary',default=str(ROOT/'../../media/maven/session/share/maven2.1'),help='DAWG data-fork file (default: original maven2.1)');a=p.parse_args()
 if a.build:build()
 ...
 e=api.match_create(str(ROOT/'resources').encode(),str(a.dictionary).encode());assert e,f'match_create rejected dictionary {a.dictionary}'
 ...
 print(json.dumps(dict(all_matched=True,dictionary=str(a.dictionary),positions=positions,ranked_records=records,kinds=kinds,scope=__doc__)))
